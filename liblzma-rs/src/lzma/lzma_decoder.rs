...
#[inline]
unsafe extern "C" fn dict_put(mut dict: *mut lzma_dict, mut byte: u8) {
    let fresh2 = (*dict).pos;
    (*dict).pos = (*dict).pos.wrapping_add(1);
...
unsafe extern "C" fn literal_init(mut probs: *mut probability, mut lc: u32, mut lp: u32) {
    ...
    while i < coders {
        *probs.offset(i as isize) = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
        i = i.wrapping_add(1);
    }
}
pub const MATCH_LEN_MIN: c_int = 2 as c_int;
pub const LEN_LOW_BITS: c_int = 3 as c_int;
pub const LEN_LOW_SYMBOLS: c_int = (1 as c_int) << LEN_LOW_BITS;
pub const LEN_MID_BITS: c_int = 3 as c_int;
pub const LEN_MID_SYMBOLS: c_int = (1 as c_int) << LEN_MID_BITS;
pub const LEN_HIGH_BITS: c_int = 8 as c_int;
pub const LEN_HIGH_SYMBOLS: c_int = (1 as c_int) << LEN_HIGH_BITS;
pub const DIST_STATES: c_int = 4 as c_int;
pub const DIST_SLOT_BITS: c_int = 6 as c_int;
pub const DIST_SLOTS: c_int = (1 as c_int) << DIST_SLOT_BITS;
...
unsafe extern "C" fn rc_read_init(
    ...
    while (*rc).init_bytes_left > 0 as u32 {
        if *in_pos == in_size {
            return LZMA_OK;
        }
        if (*rc).init_bytes_left == 5 as u32
            && *in_0.offset(*in_pos as isize) as c_int != 0 as c_int
        {
            return LZMA_DATA_ERROR;
        }
        (*rc).code = (*rc).code << 8 as c_int | *in_0.offset(*in_pos as isize) as u32;
...
unsafe extern "C" fn lzma_decode(
    ...
    let mut rc_in_end: *const u8 = in_0.offset(in_size as isize);
    let mut rc_in_fast_end: *const u8 =
        ...
        match current_block {
            ...
            12043352250568755004 => {
                ...
                if rc.code < rc_bound {
                    ...
                    (*coder).rep_len_decoder.choice = ((*coder).rep_len_decoder.choice as c_int
                        - ((*coder).rep_len_decoder.choice as c_int >> RC_MOVE_BITS))
                        as probability;
                    current_block = 6834592846991627977;
                    continue;
                }
            ...
            3996983927318648760 => {
                ...
                if rc.code < rc_bound {
                    ...
                    rc.range = rc.range.wrapping_sub(rc_bound);
                    rc.code = rc.code.wrapping_sub(rc_bound);
                    (*coder).is_rep2[state as usize] = ((*coder).is_rep2[state as usize] as c_int
                        - ((*coder).is_rep2[state as usize] as c_int >> RC_MOVE_BITS))
                        as probability;
            ...
            11808118301119257848 => {
                ...
                if rc.code < rc_bound {
                    ...
                    rc.range = rc.range.wrapping_sub(rc_bound);
                    rc.code = rc.code.wrapping_sub(rc_bound);
                    (*coder).is_rep1[state as usize] = ((*coder).is_rep1[state as usize] as c_int
                        - ((*coder).is_rep1[state as usize] as c_int >> RC_MOVE_BITS))
                        as probability;
                    current_block = 3996983927318648760;
                    continue;
                }
            ...
            1698084742280242340 => {
                ...
                if rc.code < rc_bound {
                    ...
                    rc.code = rc.code.wrapping_sub(rc_bound);
                    (*coder).is_rep0_long[state as usize][pos_state as usize] =
                        ((*coder).is_rep0_long[state as usize][pos_state as usize] as c_int
                            - ((*coder).is_rep0_long[state as usize][pos_state as usize] as c_int
            ...
            4420799852307653083 => {
                ...
                if rc.code < rc_bound {
                    ...
                    rc.range = rc.range.wrapping_sub(rc_bound);
                    rc.code = rc.code.wrapping_sub(rc_bound);
                    (*coder).is_rep0[state as usize] = ((*coder).is_rep0[state as usize] as c_int
                        - ((*coder).is_rep0[state as usize] as c_int >> RC_MOVE_BITS))
                        as probability;
                    current_block = 11808118301119257848;
                    continue;
                }
            ...
            3469750012682708893 => {
                ...
                if rc.code < rc_bound {
                    ...
                    (*coder).is_rep[state as usize] = ((*coder).is_rep[state as usize] as c_int
                        ...
                        as probability;
                    if !(!dict_is_distance_valid(&raw mut dict, 0 as size_t) as c_int as c_long
                        != 0)
                    {
                        current_block = 4420799852307653083;
                        continue;
            ...
            10510472849010538284 => {
                ...
                if rc.code < rc_bound {
                    ...
                    rc.code = rc.code.wrapping_sub(rc_bound);
                    (*coder).pos_align[offset.wrapping_add(symbol) as usize] =
                        ((*coder).pos_align[offset.wrapping_add(symbol) as usize] as c_int
                            - ((*coder).pos_align[offset.wrapping_add(symbol) as usize] as c_int
                                >> RC_MOVE_BITS)) as probability;
            ...
            15418612220330286504 => {
                ...
                rc_bound = (0 as u32).wrapping_sub(rc.code >> 31 as c_int);
                rc.code = rc.code.wrapping_add(rc.range & rc_bound);
                rep0 = (rep0 << 1 as c_int).wrapping_add(rc_bound.wrapping_add(1 as u32));
            ...
            4174862988780014241 => {
                ...
                if symbol < DIST_MODEL_START as u32 {
                    ...
                    limit = (symbol >> 1 as c_int).wrapping_sub(1 as u32);
                    rep0 = (2 as u32).wrapping_add(symbol & 1 as u32);
            ...
            13912927785247575907 => {
                ...
                if rc.code < rc_bound {
                    ...
                    (*coder).match_len_decoder.choice2 = ((*coder).match_len_decoder.choice2
                        as c_int
                        - ((*coder).match_len_decoder.choice2 as c_int >> RC_MOVE_BITS))
                        as probability;
            ...
            1138292997408115650 => {
                ...
                if rc.code < rc_bound {
                    ...
                    rc.range = rc.range.wrapping_sub(rc_bound);
                    rc.code = rc.code.wrapping_sub(rc_bound);
                    (*coder).match_len_decoder.choice = ((*coder).match_len_decoder.choice as c_int
                        - ((*coder).match_len_decoder.choice as c_int >> RC_MOVE_BITS))
                        as probability;
                    current_block = 13912927785247575907;
                    continue;
                }
            ...
            18125716024132132232 => {
                ...
                if rc.code < rc_bound {
                    ...
                    *fresh122 = (*fresh122 as c_int
                        - (*probs.offset(subcoder_index as isize) as c_int >> RC_MOVE_BITS))
                        as probability;
                    symbol = (symbol << 1 as c_int).wrapping_add(1 as u32);
                    offset &= match_bit;
                }
                len <<= 1 as c_int;
                if symbol < ((1 as c_int) << 8 as c_int) as u32 {
                    current_block = 18125716024132132232;
                    continue;
                } else {
                    current_block = 10535798129821001304;
                ...
                if rc.code < rc_bound {
                    ...
                    if state < LIT_STATES as u32 {
                        ...
                        state = if state <= STATE_LIT_SHORTREP as u32 {
                            ...
                            state.wrapping_sub(6 as u32)
                        };
                        len = (dict_get(&raw mut dict, rep0) as u32) << 1 as c_int;
                        offset = 0x100 as u32;
                        current_block = 18125716024132132232;
                        continue;
                    }
                    ...
                    rc.code = rc.code.wrapping_sub(rc_bound);
                    (*coder).is_match[state as usize][pos_state as usize] =
                        ((*coder).is_match[state as usize][pos_state as usize] as c_int
                            - ((*coder).is_match[state as usize][pos_state as usize] as c_int
                                >> RC_MOVE_BITS)) as probability;
                    current_block = 3469750012682708893;
                    continue;
            ...
            _ => {
                ...
                if rc.code < rc_bound {
                    ...
                    rc.range = rc.range.wrapping_sub(rc_bound);
                    rc.code = rc.code.wrapping_sub(rc_bound);
                    (*coder).rep_len_decoder.choice2 = ((*coder).rep_len_decoder.choice2 as c_int
                        - ((*coder).rep_len_decoder.choice2 as c_int >> RC_MOVE_BITS))
                        as probability;
                ...
                if rc.code < rc_bound {
                    ...
                    if state < LIT_STATES as u32 {
                        ...
                        if rc.code < rc_bound {
                            ...
                            *fresh27 = (*fresh27 as c_int
                                - (*probs.offset(symbol as isize) as c_int >> RC_MOVE_BITS))
                                as probability;
                            symbol = (symbol << 1 as c_int).wrapping_add(1 as u32);
                        }
                        symbol = symbol.wrapping_add(0 as u32);
                    } else {
                        state = if state <= STATE_LIT_SHORTREP as u32 {
                            state.wrapping_sub(3 as u32)
                        } else {
                        ...
                        if rc.code < rc_bound {
                            ...
                            *fresh30 = (*fresh30 as c_int
                                - (*probs.offset(t_subcoder_index as isize) as c_int
                                    >> RC_MOVE_BITS))
                                as probability;
                            symbol = (symbol << 1 as c_int).wrapping_add(1 as u32);
                            t_offset &= t_match_bit;
                    ...
                    if rc.code < rc_bound {
                        ...
                        if rc.code < rc_bound {
                            ...
                            symbol = symbol
                                .wrapping_add((-((1 as c_int) << 3 as c_int) + 2 as c_int) as u32);
                            len = symbol;
                        } else {
                            rc.range = rc.range.wrapping_sub(rc_bound);
                            rc.code = rc.code.wrapping_sub(rc_bound);
                            ...
                            if rc.code < rc_bound {
                                ...
                                rc.code = rc.code.wrapping_sub(rc_bound);
                                (*coder).match_len_decoder.choice2 =
                                    ((*coder).match_len_decoder.choice2 as c_int
                                        - ((*coder).match_len_decoder.choice2 as c_int
                                            >> RC_MOVE_BITS))
                                        as probability;
                                symbol = 1 as u32;
                                ...
                                if rc.code < rc_bound {
                                    ...
                                    rc.code = rc.code.wrapping_sub(rc_bound);
                                    (*coder).match_len_decoder.high[symbol as usize] =
                                        ((*coder).match_len_decoder.high[symbol as usize] as c_int
                                            - ((*coder).match_len_decoder.high[symbol as usize]
                                                as c_int
                                                >> RC_MOVE_BITS))
                                            as probability;
                        ...
                        if rc.code < rc_bound {
                            ...
                            *fresh86 = (*fresh86 as c_int
                                - (*probs.offset(symbol as isize) as c_int >> RC_MOVE_BITS))
                                as probability;
                            symbol = (symbol << 1 as c_int).wrapping_add(1 as u32);
                        }
                        symbol = symbol.wrapping_add(-((1 as c_int) << 6 as c_int) as u32);
                        if symbol < DIST_MODEL_START as u32 {
                            rep0 = symbol;
                        } else {
                            limit = (symbol >> 1 as c_int).wrapping_sub(1 as u32);
                            ...
                            if symbol < DIST_MODEL_END as u32 {
                                ...
                                loop {
                                    ...
                                    if rc.code < rc_bound {
                                        ...
                                        *fresh89 = (*fresh89 as c_int
                                            - (*probs.offset(symbol as isize) as c_int
                                                >> RC_MOVE_BITS))
                                            as probability;
                                        symbol = (symbol << 1 as c_int).wrapping_add(1 as u32);
                                ...
                                limit = limit.wrapping_sub(ALIGN_BITS as u32);
                                loop {
                                    rep0 = (rep0 << 1 as c_int).wrapping_add(1 as u32);
                                    if rc.range < RC_TOP_VALUE as u32 {
                                        rc.range <<= RC_SHIFT_BITS;
                                    ...
                                    (*coder).pos_align[symbol.wrapping_add(1 as u32) as usize] =
                                        ((*coder).pos_align[symbol.wrapping_add(1 as u32) as usize]
                                            ...
                                            - ((*coder).pos_align
                                                ...
                                                >> RC_MOVE_BITS))
                                            as probability;
                                    symbol = symbol.wrapping_add(1 as u32);
                                }
                                if rc.range < RC_TOP_VALUE as u32 {
                                    rc.range <<= RC_SHIFT_BITS;
                                    ...
                                    (*coder).pos_align[symbol.wrapping_add(2 as u32) as usize] =
                                        ((*coder).pos_align[symbol.wrapping_add(2 as u32) as usize]
                                            ...
                                            - ((*coder).pos_align
                                                ...
                                                >> RC_MOVE_BITS))
                                            as probability;
                                    symbol = symbol.wrapping_add(2 as u32);
                                }
                                if rc.range < RC_TOP_VALUE as u32 {
                                    rc.range <<= RC_SHIFT_BITS;
                                    ...
                                    (*coder).pos_align[symbol.wrapping_add(4 as u32) as usize] =
                                        ((*coder).pos_align[symbol.wrapping_add(4 as u32) as usize]
                                            ...
                                            - ((*coder).pos_align
                                                ...
                                                >> RC_MOVE_BITS))
                                            as probability;
                                    symbol = symbol.wrapping_add(4 as u32);
                                }
                                if rc.range < RC_TOP_VALUE as u32 {
                                    rc.range <<= RC_SHIFT_BITS;
                                    ...
                                    (*coder).pos_align[symbol.wrapping_add(8 as u32) as usize] =
                                        ((*coder).pos_align[symbol.wrapping_add(8 as u32) as usize]
                                            ...
                                            - ((*coder).pos_align
                                                ...
                                                >> RC_MOVE_BITS))
                                            as probability;
                                    symbol = symbol.wrapping_add(8 as u32);
                                }
                                rep0 = rep0.wrapping_add(symbol);
                                if rep0 == UINT32_MAX as u32 {
                                    break;
                            ...
                            != 0
                        {
                            ...
                            if rc.code < rc_bound {
                                ...
                                if rc.code < rc_bound {
                                    ...
                                    rc.code = rc.code.wrapping_sub(rc_bound);
                                    (*coder).is_rep1[state as usize] =
                                        ((*coder).is_rep1[state as usize] as c_int
                                            - ((*coder).is_rep1[state as usize] as c_int
                                                >> RC_MOVE_BITS))
                                            as probability;
                                    if rc.range < RC_TOP_VALUE as u32 {
                                    ...
                                    if rc.code < rc_bound {
                                        ...
                                        rc.code = rc.code.wrapping_sub(rc_bound);
                                        (*coder).is_rep2[state as usize] =
                                            ((*coder).is_rep2[state as usize] as c_int
                                                - ((*coder).is_rep2[state as usize] as c_int
                                                    >> RC_MOVE_BITS))
                                                as probability;
                            ...
                            if rc.code < rc_bound {
                                ...
                                symbol = symbol.wrapping_add(
                                    (-((1 as c_int) << 3 as c_int) + 2 as c_int) as u32,
                                );
                                len = symbol;
                            } else {
                                rc.range = rc.range.wrapping_sub(rc_bound);
                                ...
                                if rc.code < rc_bound {
                                    ...
                                    rc.code = rc.code.wrapping_sub(rc_bound);
                                    (*coder).rep_len_decoder.choice2 =
                                        ((*coder).rep_len_decoder.choice2 as c_int
                                            - ((*coder).rep_len_decoder.choice2 as c_int
                                                >> RC_MOVE_BITS))
                                            as probability;
                                    symbol = 1 as u32;
...
unsafe extern "C" fn lzma_decoder_reset(mut coder_ptr: *mut c_void, mut opt: *const c_void) {
    ...
    while i < STATES as u32 {
        ...
        while j <= (*coder).pos_mask {
            (*coder).is_match[i as usize][j as usize] =
                (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
            (*coder).is_rep0_long[i as usize][j as usize] =
                (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
            j = j.wrapping_add(1);
        }
        (*coder).is_rep[i as usize] = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
        (*coder).is_rep0[i as usize] = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
        (*coder).is_rep1[i as usize] = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
        (*coder).is_rep2[i as usize] = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
        i = i.wrapping_add(1);
    }
    let mut i_0: u32 = 0 as u32;
    while i_0 < DIST_STATES as u32 {
        let mut bt_i: u32 = 0 as u32;
        while bt_i < ((1 as c_int) << 6 as c_int) as u32 {
    ...
    while i_1 < (FULL_DISTANCES - DIST_MODEL_END) as u32 {
        (*coder).pos_special[i_1 as usize] = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
        i_1 = i_1.wrapping_add(1);
    }
    let mut bt_i_0: u32 = 0 as u32;
    while bt_i_0 < ((1 as c_int) << 4 as c_int) as u32 {
        (*coder).pos_align[bt_i_0 as usize] = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
        bt_i_0 = bt_i_0.wrapping_add(1);
    }
    let num_pos_states: u32 = (1 as u32) << (*options).pb;
    (*coder).match_len_decoder.choice = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
    (*coder).match_len_decoder.choice2 = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
    (*coder).rep_len_decoder.choice = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
    (*coder).rep_len_decoder.choice2 = (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
    let mut pos_state: u32 = 0 as u32;
    while pos_state < num_pos_states {
        let mut bt_i_1: u32 = 0 as u32;
        while bt_i_1 < ((1 as c_int) << 3 as c_int) as u32 {
            (*coder).match_len_decoder.low[pos_state as usize][bt_i_1 as usize] =
                (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
            bt_i_1 = bt_i_1.wrapping_add(1);
        }
        let mut bt_i_2: u32 = 0 as u32;
        while bt_i_2 < ((1 as c_int) << 3 as c_int) as u32 {
            (*coder).match_len_decoder.mid[pos_state as usize][bt_i_2 as usize] =
                (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
            bt_i_2 = bt_i_2.wrapping_add(1);
        }
        let mut bt_i_3: u32 = 0 as u32;
        while bt_i_3 < ((1 as c_int) << 3 as c_int) as u32 {
            (*coder).rep_len_decoder.low[pos_state as usize][bt_i_3 as usize] =
                (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
            bt_i_3 = bt_i_3.wrapping_add(1);
        }
        let mut bt_i_4: u32 = 0 as u32;
        while bt_i_4 < ((1 as c_int) << 3 as c_int) as u32 {
    ...
    let mut bt_i_5: u32 = 0 as u32;
    while bt_i_5 < ((1 as c_int) << 8 as c_int) as u32 {
        (*coder).match_len_decoder.high[bt_i_5 as usize] =
            (RC_BIT_MODEL_TOTAL >> 1 as c_int) as probability;
        bt_i_5 = bt_i_5.wrapping_add(1);
    }
    let mut bt_i_6: u32 = 0 as u32;
    while bt_i_6 < ((1 as c_int) << 8 as c_int) as u32 {
...
unsafe extern "C" fn lzma_decoder_init(
    ...
    if id == LZMA_FILTER_LZMA1EXT as lzma_vli {
        ...
        uncomp_size = ((*opt).ext_size_low as u64)
            .wrapping_add(((*opt).ext_size_high as u64) << 32 as c_int)
            as lzma_vli;
        allow_eopm = (*opt).ext_flags & LZMA_LZMA1EXT_ALLOW_EOPM as u32 != 0 as u32
...
pub unsafe extern "C" fn lzma_lzma_lclppb_decode(
    ...
    (*options).lc = (byte as u32).wrapping_sub((*options).lp.wrapping_mul(9 as u32));
...
#[no_mangle]
pub unsafe extern "C" fn lzma_lzma_decoder_memusage(mut options: *const c_void) -> u64 {
    if !is_lclppb_valid(options as *const lzma_options_lzma) {

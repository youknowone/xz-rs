...
unsafe extern "C" fn block_decode(
    ...
    's_177: {
        ...
        match (*coder).sequence as ::core::ffi::c_uint {
            0 => {
                    ...
                    == LZMA_OK as ::core::ffi::c_int as ::core::ffi::c_uint
                {
                    ...
                    if comp_done as ::core::ffi::c_int != 0
                        && uncomp_done as ::core::ffi::c_int != 0
                    {
                        return LZMA_DATA_ERROR;
                    }
                    if comp_done as ::core::ffi::c_int != 0 && *out_pos < out_size {
                        return LZMA_DATA_ERROR;
                    }
                    if uncomp_done as ::core::ffi::c_int != 0 && *in_pos < in_size {
                        return LZMA_DATA_ERROR;
                    }
                }
            ...
            17473121293339793080 => {
                while (*coder).compressed_size & 3 as lzma_vli != 0 {
                    ...
                    *in_pos = (*in_pos).wrapping_add(1);
                    if *in_0.offset(fresh0 as isize) as ::core::ffi::c_int
                        != 0 as ::core::ffi::c_int
                    {
                        return LZMA_DATA_ERROR;
                    }
                }
                if (*(*coder).block).check as ::core::ffi::c_uint
        ...
        lzma_bufcpy(
            ...
            in_pos,
            in_size,
            &raw mut (*(*coder).block).raw_check as *mut uint8_t,
        ...
        if !(*coder).ignore_check
            && lzma_check_is_supported((*(*coder).block).check) as ::core::ffi::c_int != 0
            && memcmp(
                &raw mut (*(*coder).block).raw_check as *mut uint8_t as *const ::core::ffi::c_void,
                &raw mut (*coder).check.buffer.u8_0 as *mut uint8_t as *const ::core::ffi::c_void,
                check_size,
            ) != 0 as ::core::ffi::c_int
        {
            return LZMA_DATA_ERROR;
...
pub unsafe extern "C" fn lzma_block_decoder_init(
    ...
    (*coder).ignore_check = if (*block).version >= 1 as uint32_t {
        (*block).ignore_check as ::core::ffi::c_int
    } else {
        false_0
    } != 0;

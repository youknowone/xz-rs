...
unsafe fn lzma2_decode(
    ...
    while *in_pos < in_size || (*coder).sequence == SEQ_LZMA {
        match (*coder).sequence {
            0 => {
                let control: u32 = *input.wrapping_add(*in_pos) as u32;
            ...
            1 => {
                (*coder).uncompressed_size = (*coder)
                    .uncompressed_size
                    .wrapping_add(((*input.wrapping_add(*in_pos) as u32) << 8) as size_t);
                *in_pos += 1;
                (*coder).sequence = SEQ_UNCOMPRESSED_2;
            }
            2 => {
                (*coder).uncompressed_size = (*coder).uncompressed_size.wrapping_add(
                    u32::from(*input.wrapping_add(*in_pos)).wrapping_add(1) as size_t,
                );
                *in_pos += 1;
                (*coder).sequence = SEQ_COMPRESSED_0;
            ...
            3 => {
                (*coder).compressed_size =
                    ((*input.wrapping_add(*in_pos) as u32) << 8) as size_t;
                *in_pos += 1;
            ...
            5 => {
                let prop_byte = *input.wrapping_add(*in_pos);
                *in_pos += 1;
